...
def create_one(v, parent_id, parent_name):
    """Just make one vm page; primarily called by create(), seen below."""

    print (v.name + ": Writing content..."),
    ...
    json_info["config_url"] = v.configuration_url
    ...
    # try:
    #     content += ("<br/>")
    #     for k in e.interfaces[0].nat_addresses.vpn_nat_addresses:
    ...
    #     content += ("NAT IP (US): " + e.interfaces[0].ip + "<br/>")

    for i in v.interfaces:
        json_info["nat_ip_us"] = i.ip
    ...
    try:
        for i in v.interfaces:
            for s in i.services:
                new_content = ("External IP: " + i.s.external_ip + "<br/>")
                json_info["external_ip"] = i.s.external_ip
    ...
    try:
        for i in v.interfaces:
            for s in i.services:
                new_content = ("External port: " + str(i.s.external_port) + "<br/>")
                json_info["external_port"] = str(i.s.external_port)
    except AttributeError:
        pass

    try:
        content += ("Internal port: " + str(e.interfaces[0].services[0].internal_port) + "<br/><br/>")
        json_info["internal_port"] = str(e.interfaces[0].services[0].internal_port)
    except AttributeError:
        pass
    ...
    for i in v.interfaces:
        new_content = ("Local IP: " + i.ip + "<br/>")
        json_info["local_ip"] = i.ip
...
def create(e, parent_id, parent_name):
    """Create vm wiki page(s)."""

    print ("Writing pages for vms.")

    for v in e.vms:
        create_one(v, parent_id, parent_name)
